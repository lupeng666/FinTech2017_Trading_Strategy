...
def test(min_t, max_t):
    """
       do regression using given min_t, max_t
       used by brute_force for parameter searching
    """
...
def print_matrix(matrix, x_list, y_list, label=""):
    """
       assistant function to print matrix
       x_list: List[num], x axis
       y_list: List[num], y axis
    """
    print("---------%s----------" % label)
    x_len = len(x_list)
    y_len = len(y_list)
    ...
    for i in range(x_len):
        print("%.4f" % y_list[i], end=' ')
        for j in range(y_len):
            print("%.4f\t" % matrix[i][j], end=' ')


def plot_asset(min_t, max_t, output="asset"):
    """
       plot *time-reward* curve and save 
    """
    config = default_config
    config["min_threshold"] = min_t
    config["max_threshold"] = max_t
...
def brute_force():
    """
       choose best parameters (min_threshold, max_threshold) by brute force grid search
       print 10 * 10 different parameter combinations
    """
    min_t_down = -0.005
    min_t_up = 0.0001
    max_t_down = 0.0002
